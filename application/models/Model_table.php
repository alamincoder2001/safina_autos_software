...
class Model_Table extends CI_Model
{
    ...
    public function generateSalesInvoice()
    {
        ...
        $sales = $this->db->query("SELECT * FROM tbl_salesmaster order by SaleMaster_SlNo desc LIMIT 1")->row();

        if ($sales) {
            $invoice = $sales->SaleMaster_InvoiceNo + 1;
            if (substr($invoice, 0, 4) != date('Y')) {
                $invoice = date('Y') . substr($invoice, 5);
            }
        } else {
            $invoice = date('Y') . '0000001';
        }
