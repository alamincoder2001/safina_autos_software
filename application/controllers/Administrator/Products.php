...
class Products extends CI_Controller {
    ...
    private function set_upload_options($path, $file_name = null)
    {   
    ...
    public function addProduct() {
        ...
        try{
            ...
            $productObj = json_decode($this->input->post('product'));
            ...
            if(!empty($_FILES)) {
                $this->upload->initialize(set_upload_options('./uploads/products', $productObj->Product_Code));
            ...
            if (!empty($_FILES['images'])) {
                ...
                for ($i = 0; $i < $cpt; $i++) {
                    ...
                    $this->upload->initialize($this->set_upload_options('./uploads/products/multiple'));
    ...
    public function updateProduct(){
        ...
        try{
            ...
            if(!empty($_FILES['userfile'])) {
                $oldProduct = $this->db->query('select * from tbl_product where Product_SlNo = ?', $productObj->Product_SlNo)->row();

                if($oldProduct->image){
                    $path = "uploads/products/$oldProduct->image";
                    if (file_exists($path)) {
                        unlink($path);
                    }
                }
            ...
            if (!empty($_FILES['images'])) {
                ...
                $oldImages = $this->db->query('select image from tbl_product_images where product_id = ?', $productObj->Product_SlNo)->result();

                $this->db->query("delete from tbl_product_images where product_id = ?", $productObj->Product_SlNo);

                foreach ($oldImages as $oldImage) {
                    $path = "uploads/products/multiple/$oldImage->image";
                    if (file_exists($path)) {
                        unlink($path);
                    }
                }
    ...
    public function getProducts(){
        ...
        $products = $this->db->query("
            ...
            left join tbl_sub_category sc on sc.id = p.sub_category_id
