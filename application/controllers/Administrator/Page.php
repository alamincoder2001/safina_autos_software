...
class Page extends CI_Controller {
    ...
    public function getCategories(){
        ...
        if(isset($data->order_by) && $data->order_by != ''){
            $order = " ORDER BY ProductCategory_SlNo $data->order_by";
        }
    ...
    public function insert_category()  {
        ...
        $nameCount = $this->db->query("SELECT * from tbl_productcategory where ProductCategory_Name = '$data->ProductCategory_Name' and status = 'a'");

        if($nameCount->num_rows() > 0){
            echo json_encode(['success' => false, 'message' => 'Name Already Exist!']);
            exit;
        }
        ...
        if(!empty($_FILES)) {
            ...
            $config['source_image'] = './uploads/categories/'. $imageName ; 
            $config['new_image'] = './uploads/categories/';
            ...
            $imageName = $uniq . $this->upload->data('file_ext');
        ...
        $this->mt->save_data('tbl_productcategory',$arr);
    ...
    public function update_category()
    {
        ...
        $nameCount = $this->db->query("SELECT * from tbl_productcategory where ProductCategory_Name = '$data->ProductCategory_Name' and status = 'a' and ProductCategory_SlNo != '$data->ProductCategory_SlNo'");
        ...
        if(!empty($_FILES)) {
            $oldImage = $this->db->query("SELECT image from tbl_productcategory where ProductCategory_SlNo = '$data->ProductCategory_SlNo'")->row()->image;
            if($oldImage){
                $path = "uploads/categories/$oldImage";
                if (file_exists($path)) {
                    unlink($path);
                }
            }
        ...
        $this->mt->update_data("tbl_productcategory", $arr, $data->ProductCategory_SlNo,'ProductCategory_SlNo');
        echo json_encode(['success' => true, 'message' => 'Updated!']); 
    }
    public function catdelete(){
        $data = json_decode($this->input->raw_input_stream);
        $sub_cats = $this->db->query("SELECT * from tbl_sub_category where category_id = $data->id")->result();
        foreach ($sub_cats as $value) {
            $this->db->where('id', $value->id)->update('tbl_sub_category', ['status' => 'd']);
        }
        $this->db->where('ProductCategory_SlNo', $data->id)->update('tbl_productcategory', ['status' => 'd']);
    ...
    public function getSubCategories(){
        ...
        $order = '';
        if(isset($data->order_by) && $data->order_by != ''){
            $order = " ORDER BY sc.id $data->order_by";
        }

        $categories = $this->db->query("SELECT sc.*, 
            c.ProductCategory_Name as category_name,
            c.image as category_image

            from tbl_sub_category sc
            join tbl_productcategory c on c.ProductCategory_SlNo = sc.category_id
            where sc.status = 'a'
            $order
        ")->result();
        echo json_encode($categories);
    ...
    public function add_sub_category()  {
        ...
        $data['content'] = $this->load->view('Administrator/add_prod_sub_category', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function insert_sub_category()  {
        $data = json_decode($this->input->post('data'));

        $nameCount = $this->db->query("SELECT * from tbl_sub_category where name = '$data->name' and status = 'a'");
        ...
        if(!empty($_FILES)) {
            ...
            $config['image_library'] = 'gd2';
            $config['source_image'] = './uploads/sub_categories/'. $imageName ; 
            $config['new_image'] = './uploads/sub_categories/';
            $config['maintain_ratio'] = TRUE;
            $config['width']    = 640;
            $config['height']   = 480;
            ...
            $path = "uploads/sub_categories/$imageName";
            if (file_exists($path)) {
                $arr['image'] = $imageName;
            }
        }

        $this->mt->save_data('tbl_sub_category',$arr);
        echo json_encode(['success' => true, 'message' => 'Success!']); 
    }
    ...
    public function update_sub_category()
    {
        ...
        $nameCount = $this->db->query("SELECT * from tbl_sub_category where name = '$data->name' and status = 'a' and id != '$data->id'");
        ...
        if(!empty($_FILES)) {
            $oldImage = $this->db->query("SELECT image from tbl_sub_category where id = '$data->id'")->row()->image;
            if($oldImage){
                $path = "uploads/sub_categories/$oldImage";
                if (file_exists($path)) {
                    unlink($path);
                }
            }
        ...
        $this->mt->update_data("tbl_sub_category", $arr, $data->id,'id');
        echo json_encode(['success' => true, 'message' => 'Updated!']); 
    }
    public function subcatdelete(){
        $data = json_decode($this->input->raw_input_stream);
        $this->db->where('id', $data->id)->update('tbl_sub_category', ['status' => 'd']);
        echo json_encode(['success' => true, 'message' => 'Deleted!']); 
    } 
    ...
    public function company_profile_Update(){
        ...
        $data['about_image']=  $this->input->post('about_image',true);
        ...
        $data['return_policy']=  $this->input->post('return_policy',true);
        // print_r($data['about_description']);
        ...
        $about_image=$this->upload->do_upload('about_image');
        $about_images = $this->upload->data();
       
        if($about_image != ""){ 
                       
            if($xx->about_image){
            unlink("./uploads/about_image/".$xx->about_image);
            }
            $data['about_image'] = $about_images['file_name'];
            
            $config['image_library'] = 'gd2';
            $config['source_image'] = $this->upload->upload_path.$this->upload->file_name;
            $config['new_image'] = 'uploads/'.'about_image/'.$this->upload->file_name;
            $config['maintain_ratio'] = FALSE;
            $config['width'] = 1000;
            $config['height'] = 1000;
            $this->load->library('image_lib', $config);
            $this->image_lib->resize();
            $data['about_image'] = $this->upload->file_name;           
        }else{
           
            $data['about_image'] = $xx->about_image; 
         }

         // website logo start
         $website_logo=$this->upload->do_upload('website_logo');
         $website_logos = $this->upload->data();
        
         if($website_logo != ""){ 
                        
             if($xx->website_logo){
             unlink("./uploads/website_logo/".$xx->website_logo);
             }
             $data['website_logo'] = $website_logos['file_name'];
             
             $config['image_library'] = 'gd2';
             $config['source_image'] = $this->upload->upload_path.$this->upload->file_name;
             $config['new_image'] = 'uploads/'.'website_logo/'.$this->upload->file_name;
             $config['maintain_ratio'] = FALSE;
             $config['width'] = 150;
             $config['height'] = 60;
             $this->load->library('image_lib', $config);
             $this->image_lib->resize();
             $data['website_logo'] = $this->upload->file_name;           
         }else{
            
             $data['website_logo'] = $xx->website_logo; 
          }
    ...
    public function Update_brand(){
        ...
        $brandname = $this->input->post('brandname');
		$branch = $this->brunch;
        $query = $this->db->query("SELECT brand_name from tbl_brand where brand_branchid = '$branch' AND brand_name = '$brandname' and brand_SiNo != $id");
        if($query->num_rows() > 0){
            $exist = false;
			echo json_encode($exist);
        }
     ...
     public function select_category_by_brand($id){
	  ...
	  public function select_category_by_branch($id){
        ...
        public function banner(){
            ...
            $data['title'] = "Banner";
            $data['content'] = $this->load->view('Administrator/banner', $data, TRUE);
            $this->load->view('Administrator/index', $data);
        }
        ...
        public function addBanner(){
            ...
            try {
                if (!empty($_FILES)) {
                    ...
                    echo json_encode('Banner Uploaded successfully');
                }
            } catch (\Throwable $th) {
        ...
        public function getBanner(){
            $banner = $this->db->query("select * from tbl_banner")->result();
        ...
        public function updateBanner($id){
            ...
            try {
                if (!empty($_FILES)) {
                    ...
                    $exist_data = $this->db->query("select * from tbl_banner where id= $id")->row();
                    if($exist_data->banner){
                        unlink("./uploads/banner/".$exist_data->banner);
                    }
        ...
        public function deleteBanner($id){
            ...
            if($exist_data->banner && file_exists('./uploads/banner/'.$exist_data->banner)){
                unlink("./uploads/banner/".$exist_data->banner);
        ...
        public function websiteMessage(){
            ...
            $data['title'] = "Website Message";
            $data['content'] = $this->load->view('Administrator/website_message', $data, TRUE);
            $this->load->view('Administrator/index', $data);
        }

        public function getMessage(){
            $data = $this->db->query("select * from tbl_message")->result();
            echo json_encode($data);
        }
        public function deleteMessage($id){
            // $this->db->query("delete tbl_message where id = $id");
