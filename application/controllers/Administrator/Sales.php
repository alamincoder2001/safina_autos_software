...
class Sales extends CI_Controller
{
    ...
    public function addSales()
    {
        ...
        try {
            ...
            if ($data->sales->salesType == 'wholesale' || ($data->sales->salesType == 'retail' &&  $data->sales->due > 0)) {
                //Send sms
    ...
    public function saleStatusUpdate()
    {
        ...
        try {
            ...
            $pre_sale = $this->db->query("SELECT * from tbl_salesmaster where SaleMaster_SlNo = $salesId")->row();

            $sales = array(
                'Status'      => $sale_status == 'Delivered' ? 'a' : 'p',
            ...
            if ($sale_status == 'Delivered') {
                ...
                foreach ($currentSaleDetails as $cartProduct) {
                    $this->db->query("
               update tbl_currentinventory 
               set sales_quantity = sales_quantity + ? 
               where product_id = ?
               and branch_id = ?
           ", [$cartProduct->SaleDetails_TotalQuantity, $cartProduct->Product_IDNo, $this->session->userdata('BRANCHid')]);
                }
            }
            ...
            $this->db->update('tbl_saledetails', $saleDetails);

            $this->db->trans_commit();
            $res = ['success' => true, 'message' => 'Status Update Successfully'];
        } catch (\Exception $e) {
            $this->db->trans_rollback();
            $res = ['success' => false, 'message' => 'Status Update Failed! Please try later'];
        }
    ...
    public function getSalesRecord()
    {
        ...
        if (isset($data->sale_status) && $data->sale_status != '') {
            $clauses .= " and sm.sale_status = '$data->sale_status'";
        }
        ...
        foreach ($sales as $sale) {
            ...
            $sale->status_label = getStatusLabel($sale->sale_status);
        }
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            $sales = array(
                ...
                'SaleMaster_Previous_Due'        => $data->sales->previousDue,
                'SaleMaster_Description'         => $data->sales->note,
                'Status'                         => $data->sales->sale_status == 'Delivered' ? 'a' : 'p',
                'sale_status'                    => $data->sales->sale_status,
                'sale_from'                      => 'Software',
                "UpdateBy"                       => $this->session->userdata("FullName"),
                'UpdateTime'                     => date("Y-m-d H:i:s"),
                "SaleMaster_branchid"            => $this->session->userdata("BRANCHid")
            ...
            if ($pre_sale->sale_status == 'Delivered') {
            ...
            foreach ($data->cart as $cartProduct) {
                ...
                if ($data->sales->sale_status == 'Delivered') {
                    $this->db->query("
                        update tbl_currentinventory 
                        set sales_quantity = sales_quantity + ? 
                        where product_id = ?
    ...
    function status_sales_record($status)
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
...
<?php
    ...
    public function printer_update()
    {
        ...
        if (!isset($_FILES['companyLogo']) || $_FILES['companyLogo']['error'] == UPLOAD_ERR_NO_FILE) {
            ...
            $id = '1';
            redirect('Administrator/sales/sales_invoice');
        } else {
