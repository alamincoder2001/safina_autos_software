	...
	<div class="row">
		<div class="col-md-12">
			<div class="table-responsive" id="stockContent">
				<table class="table table-bordered" style="display:none" v-bind:style="{display: searchType == 'current' || searchType == 'minus' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Product Id</th>
							<th>Product Name</th>
				...
				<table class="table table-bordered dataTable" style="display:none;" v-bind:style="{display: (searchType != 'current' && searchType != null) ? (searchType != 'minus' ? (searchType != 'totalStockPrint' ? '' : 'none') : 'none') : 'none'}">
					<thead>
						<tr>
							<th>SL No</th>
							<th>Product Id</th>
				...
				<table class="table table-bordered dataTable" style="display:none;" v-bind:style="{display: searchType == 'totalStockPrint' ? '' : 'none'}">
					<thead>
						<tr>
							<th>SL No</th>
							<th>Category</th>
					...
					<tbody>
						<tr v-for="(product, key) in stockPrint">
							<td> {{ key }} </td>
							<td>{{ product.ProductCategory_Name }}</td>
							<td>{{ product.brand_name }}</td>
							<td>{{ product.Product_Name }}</td>
						...
						<tr style="width:90%;font-weight:bold">
							<td colspan="4" style="text-align:right;">Total = </td>
							<td>{{ stockPrint.reduce((prev, curr) => {return prev + +curr.in}, 0)}}</td>
							<td>{{ stockPrint.reduce((prev, curr) => {return prev + +curr.out}, 0)}}</td>
							<td>{{ stockPrint.reduce((prev, curr) => {return prev + +curr.current_quantity}, 0)}}</td>
							<td></td>
						</tr>
					</tbody>
				</table>
	...
	new Vue({
		...
		data() {
			return {
				searchTypes: [{
					...
					{
						text: 'Total Stock Print',
						value: 'totalStockPrint'
					},
					{
		...
		methods: {
			getStock() {
				...
				let url = '';
				let parameters = {};
				if (this.searchType == 'current' || this.searchType == 'minus') {
				...
				axios.post(url, parameters).then(res => {
					if (this.searchType == 'current') {
						this.stock = res.data.stock.filter((obj) => obj.current_quantity != 0);
					} else if (this.searchType == 'minus') {
						this.stock = res.data.stock.filter((obj) => obj.current_quantity < 0);
					} else {
						if (this.searchType == 'totalStockPrint') {
							this.stockPrint = res.data.stock.map((obj, index) => {
								obj.in = parseFloat(obj.purchased_quantity) + parseFloat(obj.sales_returned_quantity) + parseFloat(obj.transferred_to_quantity);
								obj.out = parseFloat(obj.purchase_returned_quantity) + parseFloat(obj.damaged_quantity) + parseFloat(obj.sold_quantity) + parseFloat(obj.transferred_from_quantity);
								return obj
							})
						} else {
