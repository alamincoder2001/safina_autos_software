	...
	<div class="row" style="border-bottom: 1px solid #ccc;padding: 3px 0;">
		<div class="col-md-12">
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				...
				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'status' ? '' : 'none'}">
					<label>Status</label>
					<select class="form-control" id="status" v-model="selectedStatus">
						<option value="">All</option>
					...
					<tbody>
						<template v-for="sale in sales">
							<tr>
								...
								<td v-html="sale.status_label"></td>
								<td style="text-align:center;">
									<select class="form-control" style="padding: 0;" v-model="sale.sale_status" @change="statusChange(sale)">
										<option value="Placed">Placed</option>
										<option value="Processing">Processing</option>
										<option value="Shipped">Shipped</option>
										<option value="Delivered">Delivered</option>
				...
				<table class="record-table" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'without_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'without_details' ? '' : 'none'}">
					...
					<tbody>
						<tr v-for="sale in sales">
							...
							<td style="text-align:left;">{{ sale.sale_from }}</td>
							<td v-html="sale.status_label"></td>
							<td style="text-align:center;">
								<select class="form-control" style="padding: 0;" v-model="sale.sale_status" @change="statusChange(sale)">
									<option value="Placed">Placed</option>
...
<script>
	...
	new Vue({
		...
		data() {
			return {
				searchType: '<?php echo $status == '' ? '' : 'status'; ?>',
				...
				sales: [],
				searchTypesForRecord: ['', 'user', 'customer', 'employee', 'status'],
				searchTypesForDetails: ['quantity', 'category'],
				selectedStatus: '<?php echo $status; ?>',
			}
		},
		created() {
		...
		methods: {
			statusChange(sale) {
				axios.post('/sale_status_update', {
					id: sale.SaleMaster_SlNo,
			...
			getSalesRecord() {
				let filter = {
					...
					sale_status: this.selectedStatus,
